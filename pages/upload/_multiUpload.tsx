...
import { UploadFileType, UploadStatus } from "@w-design/upload/lib/types/FileListItem";
...
const updateList = (
  ...
  return list.map((item) =>
    item.uid === updateFile.uid ? Object.assign({ ...item }, { ...newProps }) : item
  );
};
    ...
    try {
      const result = await xhr.upload({
        ...
        onProgress: (e: ProgressEvent<EventTarget>) => {
          ...
          setFileList((list) => updateList(list, file, { percent: percentLoaded }));
        }
      });

      setFileList((list) => updateList(list, file, { status: "success", response: result }));
    } catch (err) {
      setFileList((list) => updateList(list, file, { status: "error", response: err }));
    }
  };
